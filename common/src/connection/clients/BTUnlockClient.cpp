...
void BTUnlockClient::Stop() {
    ...
    if(!m_OtherClient && m_UnlockState != UnlockState::SUCCESS && CUnlockCredential::isDeselectedSwitch) {
        if(CUnlockCredential::deselectedCount == 1) {
            timeToConnect = timeToConnect + 5000;
        } else if(timeToConnect < 1000) {
            timeToConnect =  timeToConnect + 1500;
        }
        if(secondClientConnectedFirst) {
            CUnlockCredential::deselectedCount = 0;
        }
