...
#ifdef WINDOWS
int BTUnlockClient::timeToConnect = 0;
bool BTUnlockClient::clientHadConnected = false;
bool BTUnlockClient::firstClientConnectedFirst = false;
bool BTUnlockClient::secondClientConnectedFirst = false;
bool BTUnlockClient::isAlreadyConnected = false;
bool BTUnlockClient::restartPending = false;
#endif

BTUnlockClient::BTUnlockClient(const std::string& deviceAddress, const PairedDevice& device, const bool &otherClient)
...
void BTUnlockClient::Stop() {
    if(!m_IsRunning)
        return;
#ifdef WINDOWS
    if(timeToConnect < 3000) 
        timeToConnect = timeToConnect + 3000;

    if(!m_OtherClient && m_UnlockState != UnlockState::SUCCESS && CUnlockCredential::isDeselectedSwitch) {
        std::this_thread::sleep_for(std::chrono::milliseconds(timeToConnect));
    } else if(m_OtherClient && m_UnlockState != UnlockState::SUCCESS && CUnlockCredential::isDeselectedSwitch) {
...
void BTUnlockClient::ConnectThread() {
    ...
    if(firstClientConnectedFirst)
        firstClientConnectedFirst = false;
    if(secondClientConnectedFirst)
    ...
    #ifdef WINDOWS
        if(!secondClientConnectedFirst)
            firstClientConnectedFirst = true;

        auto finish = std::chrono::high_resolution_clock::now();
        timeToConnect = std::chrono::duration_cast<std::chrono::milliseconds>(finish - start).count();
    #else
        std::this_thread::sleep_for(std::chrono::microseconds(500));
    #endif
    }
...
#ifdef WINDOWS
    if(secondClientConnectedFirst && counter < 1) {
        counter++;
        std::this_thread::sleep_for(std::chrono::microseconds(500));
        goto socketStart;
    }
...
#ifdef WINDOWS
    isAlreadyConnected = false;
    if(!clientHadConnected && CUnlockCredential::isDeselectedSwitch) {
