...
void BTUnlockClient::ConnectThread() {
    ...
    if(firstClientHasConnected)
        firstClientHasConnected = false;
    if(restartPending)
    ...
    if(m_OtherClient) {
        ...
        if(CUnlockCredential::isDeselectedSwitch) {
            ...
            if(!firstClientHasConnected && counter < 1) {
                counter++;
                SOCKET_CLOSE(m_ClientSocket);
                goto socketStart;
            }
