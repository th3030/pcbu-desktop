...
void BTUnlockClient::ConnectThread() {
    ...
    if(m_OtherClient) {
        int counter = 0;
        auto timeoutVal = std::chrono::milliseconds(3500);
        while (!hasConnected) {
            if(isAlreadyConnected && counter < 1) {
                timeoutVal = std::chrono::milliseconds(7000);
                counter++;
            }
        #ifdef WINDOWS
    ...
    if(select((int)m_ClientSocket + 1, nullptr, &fdSet, nullptr, &connectTimeout) <= 0) {
        if(numRetries < settings.clientConnectRetries && m_IsRunning && connectError <= allowedToLog) {
    ...
    isAlreadyConnected = false;
#ifdef WINDOWS
    if(!clientHadConnected && CUnlockCredential::isDeselectedSwitch && firstClientUsername == otherClientUsername) {
