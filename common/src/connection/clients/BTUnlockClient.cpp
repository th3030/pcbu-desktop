...
int BTUnlockClient::countedConnection = 0;
#endif

BTUnlockClient::BTUnlockClient(const std::string& deviceAddress, const PairedDevice& device, const bool &secondServer)
    : BaseUnlockConnection(device) {
    m_DeviceAddress = deviceAddress;
    m_Channel = -1;
    m_ClientSocket = (SOCKET)-1;
    m_IsRunning = false;
    m_SecondServer = secondServer;
}
...
void BTUnlockClient::ConnectThread() {
    ...
    if(m_SecondServer) {
        ...
        auto timeoutVal = std::chrono::milliseconds(3000);
        while (!hasConnected) {
            if(isAlreadyConnected && !extendTimeout) {
                timeoutVal = std::chrono::milliseconds(6000);
                extendTimeout = true;
            }
        #ifdef WINDOWS
    ...
    #ifdef WINDOWS
        if(CUnlockCredential::isDeselectedSwitch) {
            if(countedConnection == 0) {
                countedConnection++;
            } else {
                if(countedConnection < 1) {
                    std::this_thread::sleep_for(std::chrono::milliseconds(250));
                } else {
                    std::this_thread::sleep_for(std::chrono::milliseconds(1750));
                }

                if(countedConnection < 2) {
                    countedConnection++;
                } else {
    ...
    isAlreadyConnected = false;
#ifdef WINDOWS
    if(!m_SecondServer && m_UnlockState != UnlockState::SUCCESS && CUnlockCredential::isDeselectedSwitch)
