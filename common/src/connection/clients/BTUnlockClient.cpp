...
bool BTUnlockClient::otherClientHasConnected = false;
#ifdef WINDOWS
...
void BTUnlockClient::ConnectThread() {
    ...
    now = std::chrono::steady_clock::now();
    if(now - lastLogTime > std::chrono::seconds(7) && !restartPending && !isAlreadyConnected) {
        restartPending = true;
    ...
    connectTimeout.tv_usec = 1100000;
...
#endif
    if(!m_OtherClient && otherClientHasConnected) {
        otherClientHasConnected = false;
    #ifdef WINDOWS
        if(!CUnlockCredential::isDeselectedSwitch) {
    #endif
            std::this_thread::sleep_for(std::chrono::milliseconds(500));
